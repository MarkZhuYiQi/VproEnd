        ...
        try {
            ...
            if($orderInfo['order_coupon_selected'] > 0) {
                ...
                if (!$this->couponApi->checkUserCouponExisted($orderInfo["user_id"]))
                    throw new Exception('the coupon used by user is not existed', 'COUPON_ENTRY_NOTEXIST_CREATD_FAILURE');
                ...
                if (!count($coupon_valid))
                    throw new Exception('', 'COUPON_NOTEXIST_USED');

                //判断优惠券是否在可使用时间段
                if ($coupon_valid['coupon_start_date'] > time() || $coupon_valid['coupon_end_date'] < time())
                    throw new Exception('the coupon does not match with date which could be used', 'COUPON_DATE_MISMATCH');

                //判断总价是否达到优惠券要求
                if ($coupon_valid['coupon_limit'] > $res['order_price'])
                    throw new Exception('total price does not reach the limit', 'COUPON_LIMIT_MISMATCH');
            ...
            if ($res['summary_price'] < 0)
                throw new Exception('total price lower than 0', 'ORDER_PRICE_MINUS');
...
//                $order_id = IdController::getOrderId($this->redis);
                ...
                // 将课程购买信息放入子订单列表中
                if(!$this->putSubOrder($orderInfo['courses'], $order_id))throw new Exception('sub order error!', $this->params['SUB_ORDER_INSERT_ERROR']);
                // 订单日志
