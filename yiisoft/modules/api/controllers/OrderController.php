...
//    public $modelClass='app\models\VproOrder';
    ...
    function behaviors(){
        ...
        $behaviors['authenticator']=[
             ...
             */
            'except'=>['get-order-courses', 'get-my-courses', 'test', 'pay']
        ];
        return $behaviors;
    }
    ...
    function putOrder($orderInfo) {
        if(count($orderInfo) > 0) {
            ...
            try{
                ...
                $subject = mb_substr(preg_replace("/\s/","",implode("", $subject)),0,32, 'utf-8');
//                $order_id = IdController::getOrderId($this->redis);
    ...
    function actionPay()
    {
        if(!$body = $this->checkParams(['orderId'], 'get')) return json_encode($this->returnInfo('OrderId missing', 'PARAMS_ERROR'));
        if (strlen($body['orderId']) !== 19 || !preg_match('/\d{19}/', $body['orderId']))return json_encode($this->returnInfo('orderId error', 'PARAMS_ERROR'));
        $vproOrder = new VproOrder();
        $res = $vproOrder::find()->where(['order_id' => $body['orderId']])->limit(1)->asArray()->one();
        if($res === null) {
            Common::failScoring($this->params['DB_MISS']);
            return json_encode($this->returnInfo('this order does not exist', $this->params['ORDER_NOT_EXIST']));
        }
        $pay = $this->genPayUrl($res['order_id'], $res['order_title'], $res['order_price'], 1800, $res['order_id']);
        return json_encode($this->returnInfo($pay));
    }
    /**
    ...
    function actionGetMyCourses()
    {
        $user_id = $this->request->get('user_id', false);
        if (!$user_id) return json_encode($this->returnInfo('params missing', $this->params['PARAMS_ERROR']));
        $payStatus = 1;
        $personalKey = 'UserCourses:'.$user_id;
        /*if ($this->redis->ttl($orderKey) > 0) {
            return json_encode($this->returnInfo(json_decode($this->redis->get($orderKey))));
        }*/
        $orders_sql = <<<q
...
DESC
q;
        $res = $this->db->createCommand($orders_sql)
            ->bindValues([
                'payStatus' =>  $payStatus
            ])
            ->queryAll();
        // 存储方式为 UserCourses:[用户id]:[]
        $this->redis->setEx($personalKey, rand(30, 60), json_encode($res));
        return json_encode($this->returnInfo($res));
    }
